...
**Fase atual:** US-001 concluída — setup base do Next.js pronto; próxima etapa: Hero section (US-002)
...
- [x] Página placeholder "Em construção 🚧" e layout raiz com `lang="pt-BR"` e fonte Geist
...
1. **US-002** — Hero section (foto de perfil + resumo/currículo)
2. **US-003** — Seção de resumo do projeto guarda-chuva
3. **US-004** — Componente reutilizável de card de projeto/sistema
4. **US-005** — Listagem/grid de cards de projetos
5. **US-006** — Responsividade mobile
6. **US-007** — SEO básico (meta tags, Open Graph, favicon)
7. **US-009** — Testes unitários dos componentes principais
8. **US-008** — Configuração de deploy no Vercel com domínio `gusmaolima.com`

> **Nota:** US-009 pode ser desenvolvida incrementalmente junto com US-002 a US-005 (testar cada componente ao criá-lo). A listagem acima coloca testes consolidados após as features, mas a abordagem co-localizada (teste junto ao componente) é preferível.
...
| # | Pendência | Impacto |
|---|-----------|---------|
| 1 | Foto de perfil e textos finais (currículo resumido, descrição do projeto guarda-chuva) | Bloqueia conteúdo real na US-002 e US-003; placeholders podem ser usados inicialmente |
